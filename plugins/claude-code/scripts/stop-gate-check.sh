...
#   2. jq not installed -> allow stop (can't parse hook input)
...
# Guard 2: jq required for JSON parsing
...
cd "$CWD" || exit 0
GATE_OUTPUT=$(assay gate run --all --json 2>/dev/null)
GATE_EXIT=$?
...
if [ "$MODE" = "warn" ]; then
  # Warn mode: allow stop but surface warning to the agent
  jq -n --arg count "$FAILED_COUNT" '{
    reason: "Warning: quality gates are failing (\($count) criteria). Run /assay:gate-check to review."
  }'
  exit 0
fi
