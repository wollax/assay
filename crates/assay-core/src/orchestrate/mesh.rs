//! Stub executor for Mesh mode.
//!
//! `run_mesh()` accepts the same signature as `run_orchestrated()` but returns
//! an empty `OrchestratorResult` immediately. Sessions with non-empty
//! `depends_on` emit a `tracing::warn!` because dependency edges are ignored in
//! Mesh mode.
...
/// Stub for Mesh-mode execution.
///
/// This is a placeholder that will be replaced by a real peer-mesh executor in
/// a future milestone. For now it:
/// - warns per session if `depends_on` is non-empty (dependency edges are
///   ignored in Mesh mode)
...
mod tests {
    ...
    #[test]
    fn run_mesh_returns_empty_result() {
        let manifest = make_manifest(vec![make_session("some-spec", vec![])]);
        let config = OrchestratorConfig::default();
        let result = run_mesh(&manifest, &config, &make_pipeline_config(), &make_runner()).unwrap();
