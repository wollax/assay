...
**M012 complete.** All 5 slices shipped. S01 cleaned up M011 leftover work: three-way tracing subscriber init (D158), full eprintln! migration (D159), flaky test timeout fix. R061 and R062 now validated. S02 delivered the full tracker contract layer: TrackerSource trait (RPITIT), TrackerConfig with deny_unknown_fields, ServerConfig integration with collected validation, load_template_manifest() + issue_to_manifest() + MockTrackerSource, StateBackendConfig mirror enum, and JobManifest.state_backend field. S03 delivered GithubTrackerSource: GhClient trait with 4 RPITIT methods, SubprocessGhClient via gh CLI subprocess, MockGhClient VecDeque test double, GithubTrackerSource<G> with atomic label transitions (D157/D166) + idempotent label creation. S04 delivered LinearTrackerSource: LinearClient trait with 5 RPITIT GraphQL methods, ReqwestLinearClient production impl, MockLinearClient VecDeque test double, LinearTrackerSource<L> with ensure_labels() UUID caching and two-mutation transition_state(). S05 wired TrackerPoller as a 6th tokio::select! arm in smelt serve (D171 AnyTrackerSource enum dispatch, D172 non-fatal poll errors, D173 pending() placeholder when no tracker), forwarded state_backend through to SmeltRunManifest TOML (R075 validated), added TUI Source column, and updated examples/server.toml and README.md. 398 workspace tests, 0 failures. Live UAT with real gh CLI and Linear API deferred.
...
| M012 | Tracker-Driven Autonomous Dispatch | ✅ Complete (2026-03-28, pending live UAT) |
