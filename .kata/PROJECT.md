...
v0.9.0-dev. M001–M012 complete. ~26K lines of Rust across 7 crates. 1503 tests passing (default workspace). 72 requirements validated, 0 active. `assay-tui` is a full Ratatui application with dashboard, in-TUI authoring wizard, spec browser, provider configuration, agent spawning with live output streaming, slash command overlay, MCP server configuration panel, PR status badges with background polling, and gate history analytics screen. Full OpenTelemetry tracing stack: structured leveled events (zero eprintln!), pipeline and orchestration spans, JSON file trace export under `.assay/traces/`, feature-flagged OTLP export, and W3C TRACEPARENT subprocess propagation. `assay-backends` crate provides `LinearBackend`, `GitHubBackend`, and `SshSyncBackend` behind feature flags; `backend_from_config()` factory fn wired into all CLI/MCP construction sites.
...
**M010 (complete):** Pluggable State Backend — S01 delivered StateBackend trait (7 sync methods, object-safe), CapabilitySet flags struct, LocalFsBackend skeleton, StateBackendConfig enum (LocalFs + Custom), contract tests proving trait object construction, D149 documenting the deliberate D001 exception. R071 validated. S02 delivered LocalFsBackend real method bodies (push_session_event with atomic tempfile-rename, read_run_state, save_checkpoint_summary, send_message, poll_inbox, annotate_run), Arc<dyn StateBackend> on OrchestratorConfig (manual Clone/Debug impls), replacement of all 11 persist_state() callsites across executor/mesh/gossip, RunManifest.state_backend field with backward-compat serde, schema snapshot split for orchestrate/non-orchestrate feature variants, CLI/MCP construction sites updated with explicit LocalFsBackend. R072/R073 validated. S03 delivered CapabilitySet capability checks in run_mesh() (supports_messaging guards routing thread) and run_gossip() (supports_gossip_manifest guards PromptLayer injection and manifest writes), NoopBackend test helper, and two integration tests proving graceful degradation. R074 validated. S04 delivered plugins/smelt-agent/ with AGENTS.md and three skills (run-dispatch, backend-status, peer-message) documenting the backend-aware API surface. R075 validated. 1488 tests.

**M012 (complete):** Checkpoint Persistence on Remote Backends — S01 delivered `GuardDaemon.backend: Arc<dyn StateBackend>` field behind orchestrate feature gate, dual constructors (with/without backend), `save_checkpoint_routed()` testable routing surface, dual `start_guard()` signatures in mod.rs, `SpyBackend` test double, two contract tests proving backend routing when `supports_checkpoints` is true/false, CLI `handle_guard_start` wired with `Arc::new(LocalFsBackend::new(assay))` (no behavior change for existing users). D175 and D176 appended to DECISIONS.md. R080 validated. 1503 tests.

**M011 (complete):** Concrete Remote Backends — S01 delivered assay-backends crate scaffold with linear/github/ssh feature flags, StateBackendConfig::Linear/GitHub/Ssh variants, schema snapshots updated, backend_from_config() factory fn dispatching all variants. R079 validated. 1497 tests. S02 delivered LinearBackend implementing all 7 StateBackend methods (reqwest::blocking GraphQL client, .linear-issue-id file lifecycle, annotate_run [assay:manifest] tag, D164 capability flags), 8 mockito contract tests, factory dispatch updated. R076 validated. 1501 tests. S03 delivered GitHubBackend implementing all 7 StateBackend methods via gh CLI subprocess, .github-issue-number file lifecycle, 8 mock-binary contract tests, CapabilitySet::none(). R077 validated. 1501 tests. S04 delivered SshSyncBackend implementing all 7 StateBackend methods via scp/ssh Command::arg() chaining (injection-safe), CapabilitySet::all(), 9 mock scp/ssh contract tests, and wired backend_from_config() into all 6 CLI/MCP OrchestratorConfig construction sites (zero hardcoded LocalFsBackend::new at manifest-dispatch callsites). R078/R079 fully validated. 1526 tests (all features).
...
- [x] M012: Checkpoint Persistence on Remote Backends — GuardDaemon accepts Arc<dyn StateBackend>; try_save_checkpoint routes through backend when supports_checkpoints=true; start_guard() API updated; CLI passes LocalFsBackend; contract tests via SpyBackend; R080 validated; 1503 tests (complete)
- [x] M013: Tech Debt & Deferred Features — S01 ✅ GitHubBackend construction validation (R081); S02 ✅ TUI trace viewer (R066); S03 ✅ OTel metrics (R067); S04 ✅ Wizard runnable criteria (R082) — all 4 slices complete, 1525 tests
