...
**M012 complete.** All 5 slices shipped. S01 cleaned up M011 leftover work: three-way tracing subscriber init (D158), full eprintln! migration (50 calls across 6 files), flaky test timeout fix from 10s to 30s. R061 and R062 now validated. S02 delivered the full tracker contract layer: TrackerSource trait (RPITIT, D019), TrackerConfig with deny_unknown_fields, ServerConfig integration with collected validation (D018), load_template_manifest() + issue_to_manifest() (D161) + MockTrackerSource, StateBackendConfig mirror enum (D163), JobManifest.state_backend field (D160). S03 delivered GithubTrackerSource: GhClient trait with 4 RPITIT methods, SubprocessGhClient via gh CLI subprocess, MockGhClient VecDeque test double, GithubTrackerSource<G: GhClient> with atomic label transitions (D157/D166, single gh issue edit) + idempotent ensure_labels(). S04 delivered LinearTrackerSource: LinearClient trait with 5 RPITIT GraphQL methods, ReqwestLinearClient production impl (reqwest promoted to production dep), MockLinearClient VecDeque test double, LinearTrackerSource<L: LinearClient> with ensure_labels() UUID caching (D169) and two-mutation transition_state() (D170). S05 wired TrackerPoller as a 6th tokio::select! arm in smelt serve: AnyTrackerSource enum dispatch (D171), non-fatal poll errors (D172), pending() placeholder when no tracker (D173); forwarded state_backend through SmeltRunManifest TOML (R075 validated); added TUI Source column; updated examples/server.toml and README.md. 398 workspace tests, 0 failures. R072, R073, R074, R075 validated. R070 (GitHub) and R071 (Linear) remain active — live UAT with real gh CLI and Linear API deferred.
...
| M012 | Tracker-Driven Autonomous Dispatch | ✅ Complete (2026-03-28, live UAT deferred) |
