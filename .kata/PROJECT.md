...
v0.4.0 on main. ~20K lines of Rust across 6 crates. M001–M004 complete. 1271 tests passing.
...
**M005 (in progress, S01–S05 complete):** Spec-driven development core — S01: Milestone/ChunkRef/MilestoneStatus types, atomic I/O, milestone_list/milestone_get MCP tools. S02: full development cycle state machine, cycle_status/cycle_advance/chunk_status MCP tools, guarded phase transitions, assay milestone status/advance CLI. S03: guided authoring wizard, assay plan CLI, milestone_create/spec_create MCP tools. S04: gate-gated PR workflow, pr_create MCP tool, assay pr create CLI, Verify→Complete transition. S05: Claude Code plugin upgrade — /assay:plan (interview-first), /assay:status, /assay:next-chunk skills; rewritten CLAUDE.md; cycle-aware Stop hook (cycle-stop-check.sh) naming failing chunks in block reason; PostToolUse names active chunk; plugin.json v0.5.0. 47 requirements validated (R001–R029, R034–R047). 1331+ tests. S06 (Codex plugin) remains.
...
- **assay-cli**: CLI binary — init, spec, gate, worktree, context, checkpoint, guard, mcp, run, harness subcommands
- **assay-mcp**: MCP server — 22 tools (spec, gate, worktree, session, merge, context, orchestrate)
...
- [ ] M005: Spec-Driven Development Core — S01+S02 complete (types/I/O/cycle state machine/MCP/CLI); S03–S06 in progress (wizard, PR workflow, plugins)
