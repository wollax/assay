...
**M010 (in progress):** Pluggable State Backend — S01 delivered StateBackend trait (7 sync methods, object-safe), CapabilitySet flags struct, LocalFsBackend skeleton, StateBackendConfig enum (LocalFs + Custom), contract tests proving trait object construction, D149 documenting the deliberate D001 exception. R071 validated. S02 delivered LocalFsBackend real method bodies (push_session_event with atomic tempfile-rename, read_run_state, save_checkpoint_summary, send_message, poll_inbox, annotate_run), Arc<dyn StateBackend> on OrchestratorConfig (manual Clone/Debug impls), replacement of all 11 persist_state() callsites across executor/mesh/gossip, RunManifest.state_backend field with backward-compat serde, schema snapshot split for orchestrate/non-orchestrate feature variants, CLI/MCP construction sites updated with explicit LocalFsBackend. R072/R073 validated. 1481 tests. S03 delivered NoopBackend test helper (all capabilities false, all methods no-op), capability guard in run_mesh() skipping routing thread when supports_messaging is false, capability guard in run_gossip() skipping PromptLayer injection and all persist_knowledge_manifest callsites when supports_gossip_manifest is false, tracing::warn! events with structured capability/mode fields at each guard. R074 validated. 1486 tests.
...
- [ ] M010: Pluggable State Backend — S01 complete (StateBackend trait, CapabilitySet, LocalFsBackend skeleton, StateBackendConfig schema locked, R071 validated, 1471 tests). S02 complete (LocalFsBackend real implementations, Arc<dyn StateBackend> on OrchestratorConfig, all persist_state() callsites replaced, RunManifest.state_backend field added, R072/R073 validated, 1481 tests). S03 complete (NoopBackend, capability guards in run_mesh()/run_gossip(), degradation integration tests, R074 validated, 1486 tests). S04 in progress.
