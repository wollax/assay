...
- [ ] **T01: Define `TuiEvent`, `AgentRunStatus`, `Screen::AgentRun`, and `launch_agent_streaming` — with failing tests** `est:1h`
...
- [ ] **T02: Implement `handle_agent_line`, `handle_agent_done`, and `draw_agent_run`** `est:1h`
...
- [ ] **T03: Refactor `run()` to channel-based event loop + wire `r` key handler** `est:1.5h`
